...
logger = logging.getLogger('microbyte-platform')
...
set(MICROBYTE_SDK $microbyte_sdk)
set(MICROBYTE_OS $microbyte_os)
set(MICROBYTE_OS_VER $microbyte_os_ver)
set(MICROBYTE_DEVICE $microbyte_device)
set(MICROBYTE_MIDDLEWARE $microbyte_mw_list)
set(MICROBYTE_BUILD_SYS_MIN_VER $microbyte_build_sys_min_ver)
'''
    ...
    with open(autogen_file, 'wt') as fh:
        fh.write(
            cmake_template.safe_substitute(
                microbyte_sdk=_sdk,
                microbyte_os=_os,
                microbyte_os_ver=_os_version,
                microbyte_device=_device,
                microbyte_mw_list=' '.join(_mw_list),
                microbyte_build_sys_min_ver=BUILD_SYS_MIN_VER,
            )
        )
    ...
    def apply_patch(self):
        if self.patch_file:
            ...
            if not is_integrated and self.source.__class__.__name__ == 'GitSource':
                check_cmd(['git', 'config', 'user.name', 'microbyte-platform'], cwd=self.destination, **self.stream_kwargs)
...
class Target(Element):
    ...
    For supported targets run::
        ...
        microbyte-platform deploy -h

    Target's operations are

    * Writing deployment instructions to stdout/file.

    * Fetching the required target-dependent sources according to microbyte-platform.json.

    * Deleting fetched target-dependent sources according to microbyte-platform.json.

    * Apply patches if needed on the deployed sources according to microbyte-platform.json.

    :param name: Target name
    :param data: Dictionary describing the target (read from microbyte-platform.json)
    :param stream_kwargs:
        * *stdout* --
          Standard output handle
        * *stderr* --
    ...
    def fetch_elements(self):
        """
        Fetch the required target-dependent sources for the target according to microbyte-platform.json
        """
        self.fetch()
        self.os.fetch()
        self.device.fetch()
    ...
    def delete_elements(self):
        """
        Delete the required target-dependent sources of the target according to microbyte-platform.json
        """
        self.os.delete()
        self.device.delete()
        for mw in self.middleware:
    ...
    def patch_elements(self):
        """
        Apply patches if needed on the deployed sources of the target according to microbyte-platform.json
        """
        self.apply_patch()
        self.os.apply_patch()
...
@click.option(
    ...
    default=os.path.join(SCRIPT_DIR, 'microbyte-platform.json'),
    help='Path to a .json file containing the supported targets configuration.\n'
         'Default is %s' % os.path.normpath(os.path.join(SCRIPT_DIR, 'microbyte-platform.json'))
)
...
@cli.command(
    ...
    short_help='Deploy microbyte files (run "%s deploy -h" for help)' % PROG_NAME
)
@click.option(
    '--target',
    'target_name',
...
def generate(config, target_name):
    ...
    if not hasattr(config, 'target_name'):
        ...
        shutil.copy(
            os.path.join(PLATFORM_ROOT, 'MicroByteCmake.txt'),
